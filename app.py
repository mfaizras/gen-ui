...
class GenUI:
    ...
    def add_to_history(self,chat):
        if len(self.st.session_state.messages) >= 10:
            self.st.session_state.messages.pop(0)
    ...
    def generate_response(self,input):
        ...
        messageHistory.extend(self.st.session_state.messages)
    ...
    def main(self):
        ...
        #         self.client = GoogleGenerativeAI(api_key=gemini_api_key_input,model=selected_model)
        #     else:
        ...
        for message in self.st.session_state.messages:
            if isinstance(message, HumanMessage):
                ...
                with self.st.chat_message("assitant"):
                    self.st.markdown(message.content)
